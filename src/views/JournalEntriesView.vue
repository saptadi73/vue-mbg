...
const journalSearchText = (item: unknown) => {
  ...
  return [row.entry_number, row.reference, row.description, row.source_module, row.status]
    .filter(Boolean)
    .join(' ')
}
    ...
    <section class="grid gap-4 xl:grid-cols-4">
      <article class="glass-panel p-5">
        ...
        <p class="mt-3 font-display text-3xl text-app-heading">
          {{ journalState.data.value?.total || 0 }}
        </p>
        <p class="mt-2 text-sm text-app-body">
          Total header jurnal yang tersimpan di yayasan aktif.
        </p>
      </article>
      <article class="glass-panel p-5">
        ...
        <p class="mt-2 text-sm text-app-body">
          Nilai debit dari form jurnal manual yang sedang disusun.
        </p>
      </article>
      <article class="glass-panel p-5">
        ...
        <p class="mt-2 text-sm text-app-body">
          Nilai credit harus seimbang sebelum jurnal disimpan.
        </p>
      </article>
      <article class="glass-panel p-5">
        <p class="text-sm text-app-muted">Balance check</p>
        <p class="mt-3 font-display text-3xl text-app-heading">
          {{ totalDebit === totalCredit ? 'Balanced' : 'Mismatch' }}
        </p>
        <p class="mt-2 text-sm text-app-body">
          Frontend akan menolak simpan bila debit dan credit belum sama.
        </p>
      </article>
    </section>
        ...
        <form class="mt-6 grid gap-4" @submit.prevent="handleCreateJournal">
            ...
            <div class="flex items-center justify-between gap-3">
              <div>
                ...
                <p class="mt-1 text-sm text-app-body">
                  Minimal harus ada satu debit dan satu credit.
                </p>
              </div>
            ...
            <div class="mt-4 grid gap-4">
              ...
              >
                <select
                  ...
                  class="toolbar-input"
                  @change="syncLineAccount(index, line.account_code)"
                >
                ...
                <input
                  ...
                  type="number"
                />
                <button class="secondary-button" type="button" @click="removeLine(index)">
                  Hapus
                </button>
              </div>
            </div>
          ...
          <div class="flex justify-end">
            <button
              ...
              :disabled="saving || totalDebit !== totalCredit || !form.lines.length"
              type="submit"
            >
              ...
              <tr v-for="item in items" :key="(item as JournalEntryRecord).id">
                ...
                <td>
                  <RouterLink
                    ...
                    :to="`/accounting/journal-entries/${(item as JournalEntryRecord).id}`"
                    >Detail</RouterLink
                  >
                </td>
              </tr>
...
.journal-entry-table {
  min-width: 1080px;
  --journal-col-entry: 170px;
  --journal-col-date: 140px;
}
...
.journal-entry-table th:nth-child(1),
.journal-entry-table td:nth-child(1) {
  min-width: var(--journal-col-entry);
}

.journal-entry-table th:nth-child(2),
.journal-entry-table td:nth-child(2) {
  min-width: var(--journal-col-date);
}

.journal-entry-table th:nth-child(3),
.journal-entry-table td:nth-child(3) {
  min-width: 180px;
}

.journal-entry-table th:nth-child(5),
.journal-entry-table td:nth-child(5),
.journal-entry-table th:nth-child(6),
.journal-entry-table td:nth-child(6) {
  text-align: right;
}
